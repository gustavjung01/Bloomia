...
export function SettingsPage() {
  ...
  async function refreshManualSetupData() {
    try {
      setError('');
      const [shopRecord, categoryRows, unitRows, supplierRows, itemRows] = await Promise.all([
        getShopSettings(),
        listCategories(),
        listUnits(),
        listSuppliers(),
      ...
      setShopForm({
        ...
        logoPath: shopRecord?.logo_path ?? '',
        invoiceFooter: shopRecord?.invoice_footer ?? '',
      });
  ...
  useEffect(() => {
    void refreshManualSetupData();
  }, []);

  const categoryOptions = useMemo(
    () => [{ label: 'Không chọn', value: '' }, ...categories.map((category) => ({ label: category.name, value: category.id }))],
    [categories],
  );

  const unitOptions = useMemo(
    () => [{ label: 'Không chọn', value: '' }, ...units.map((unit) => ({ label: `${unit.name} (${unit.symbol})`, value: unit.id }))],
    [units],
  );
  ...
  async function handleSaveShop() {
    ...
    await runAction('Đã lưu thông tin shop.', async () => {
  ...
  async function handleSaveCategory() {
    if (!categoryForm.name.trim()) {
      setError('Tên nhóm hàng là bắt buộc.');
      return;
    }

    await runAction('Đã lưu nhóm hàng.', async () => {
  ...
  async function handleSaveUnit() {
    if (!unitForm.name.trim() || !unitForm.symbol.trim()) {
      setError('Tên đơn vị tính và ký hiệu là bắt buộc.');
      return;
    }

    await runAction('Đã lưu đơn vị tính.', async () => {
  ...
  async function handleSaveSupplier() {
    ...
    await runAction('Đã lưu nhà cung cấp.', async () => {
  ...
  async function handleSaveItem() {
    if (!itemForm.name.trim()) {
      setError('Tên hàng hóa/dịch vụ là bắt buộc.');
      return;
    }

    const defaultSalePrice = Number(itemForm.defaultSalePrice);
    if (Number.isNaN(defaultSalePrice) || defaultSalePrice < 0) {
      setError('Giá bán gợi ý phải là số không âm.');
      return;
    }
    ...
    await runAction('Đã lưu hàng hóa/dịch vụ.', async () => {
      await saveItem({
        ...
        defaultSalePrice,
        isStockTracked: itemForm.itemType === 'service' ? false : itemForm.isStockTracked,
        note: itemForm.note,
      });
  ...
  async function handleArchiveItem(id: string) {
    await runAction('Đã ẩn hàng hóa/dịch vụ.', async () => {
    ...
    <>
      <div className="page-title-row">
        <div>
          ...
          <h2>Setup thủ công cho chủ tiệm</h2>
        </div>
        <PillTabs
          value={activeTab}
          onChange={setActiveTab}
          options={[
            { label: 'Thông tin shop', value: 'shop' },
            { label: 'Danh mục nền', value: 'catalog' },
            { label: 'Hàng hóa & dịch vụ', value: 'items' },
          ]}
        />
      </div>
    ...
    <div className="page-grid">
      <SoftCard className="span-7" title="Thông tin shop" description="Dùng cho hóa đơn, phiếu giao hàng và báo cáo.">
        <div className="setup-form-grid">
          ...
          <TextField label="Logo path local" value={shopForm.logoPath} placeholder="Ví dụ: C:\\Bloomia\\logo.png" onChange={(event) => setShopForm((form) => ({ ...form, logoPath: event.target.value }))} />
      ...
      <SoftCard className="span-5" title="Preview hóa đơn" description="Thông tin này sẽ đi vào template in ở Phase C.">
        <div className="invoice-preview-card">
          <strong>{shopForm.name || shop?.name || 'Tên shop'}</strong>
          <span>{shopForm.phone || 'SĐT shop'}</span>
          <span>{shopForm.address || 'Địa chỉ shop'}</span>
          <hr />
          <p>{shopForm.invoiceFooter || 'Cảm ơn quý khách đã ghé Bloomia.'}</p>
        </div>
      </SoftCard>
    </div>
  );
...
interface CatalogSetupPanelProps {
  ...
  onSaveUnit: () => void;
  onSaveSupplier: () => void;
  onArchive: (kind: CatalogKind, id: string) => void;
}
...
function CatalogSetupPanel({
  ...
  onSaveSupplier,
  onArchive,
}: CatalogSetupPanelProps) {
  return (
    <div className="page-grid">
      <SoftCard className="span-4" title="Nhóm hàng" description="Hoa tươi, lá phụ, phụ liệu, dịch vụ...">
        <div className="setup-form-grid">
          <TextField label="Tên nhóm" value={categoryForm.name} onChange={(event) => setCategoryForm((form) => ({ ...form, name: event.target.value }))} />
          <TextField label="Thứ tự" type="number" value={categoryForm.sortOrder} onChange={(event) => setCategoryForm((form) => ({ ...form, sortOrder: Number(event.target.value) }))} />
          <Button onClick={onSaveCategory}>{categoryForm.id ? 'Cập nhật nhóm' : 'Thêm nhóm'}</Button>
        </div>
        <CatalogList
          rows={categories.map((category) => ({ id: category.id, title: category.name, meta: `Thứ tự ${category.sort_order}` }))}
          onEdit={(id) => {
            const category = categories.find((row) => row.id === id);
            if (category) setCategoryForm({ id: category.id, name: category.name, sortOrder: category.sort_order });
          }}
          onArchive={(id) => onArchive('category', id)}
        />
      </SoftCard>

      <SoftCard className="span-4" title="Đơn vị tính" description="Cành, bó, tờ, cái, lần, chuyến...">
        <div className="setup-form-grid">
          <TextField label="Tên đơn vị" value={unitForm.name} onChange={(event) => setUnitForm((form) => ({ ...form, name: event.target.value }))} />
          <TextField label="Ký hiệu" value={unitForm.symbol} onChange={(event) => setUnitForm((form) => ({ ...form, symbol: event.target.value }))} />
          <Button onClick={onSaveUnit}>{unitForm.id ? 'Cập nhật đơn vị' : 'Thêm đơn vị'}</Button>
        </div>
        <CatalogList
          rows={units.map((unit) => ({ id: unit.id, title: unit.name, meta: unit.symbol }))}
          onEdit={(id) => {
            const unit = units.find((row) => row.id === id);
            if (unit) setUnitForm({ id: unit.id, name: unit.name, symbol: unit.symbol });
          }}
          onArchive={(id) => onArchive('unit', id)}
        />
      </SoftCard>

      <SoftCard className="span-4" title="Nhà cung cấp" description="Lưu đầu mối nhập hoa và phụ liệu.">
        <div className="setup-form-grid">
          <TextField label="Tên NCC" value={supplierForm.name} onChange={(event) => setSupplierForm((form) => ({ ...form, name: event.target.value }))} />
          <TextField label="SĐT" value={supplierForm.phone} onChange={(event) => setSupplierForm((form) => ({ ...form, phone: event.target.value }))} />
          <TextField label="Địa chỉ" value={supplierForm.address} onChange={(event) => setSupplierForm((form) => ({ ...form, address: event.target.value }))} />
          <TextArea label="Ghi chú" value={supplierForm.note} onChange={(event) => setSupplierForm((form) => ({ ...form, note: event.target.value }))} />
          <Button onClick={onSaveSupplier}>{supplierForm.id ? 'Cập nhật NCC' : 'Thêm NCC'}</Button>
        </div>
        <CatalogList
          rows={suppliers.map((supplier) => ({ id: supplier.id, title: supplier.name, meta: supplier.phone ?? 'Chưa có SĐT' }))}
          onEdit={(id) => {
...
function CatalogList({ rows, onEdit, onArchive }: CatalogListProps) {
  return (
    <div className="setup-list">
      {rows.length === 0 && <p className="setup-muted">Chưa có dữ liệu.</p>}
      {rows.map((row) => (
        <div className="setup-list-row" key={row.id}>
          <div>
...
interface ItemSetupPanelProps {
  ...
  items: ItemRecord[];
  itemFilter: ItemType | 'all';
  setItemFilter: React.Dispatch<React.SetStateAction<ItemType | 'all'>>;
